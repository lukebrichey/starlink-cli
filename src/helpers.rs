...
pub fn print_color_percentage(percentage: f32) {
    if percentage >= 0.0 && percentage < 6.0 {
        println!("{}{:.2}%{}", color::Fg(color::Green), percentage, color::Fg(color::Reset))
    } else if percentage >= 6.0 && percentage < 11.0 {
        println!("{}{:.2}%{}", color::Fg(color::Yellow), percentage, color::Fg(color::Reset))
    } else {
        println!("{}{:.2}%{}", color::Fg(color::Red), percentage, color::Fg(color::Reset))
    }
}